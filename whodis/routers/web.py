...
async def dashboard(
    ...
    """Main dashboard page."""
    from whodis.models import DetectionLog, APIKey
    from datetime import datetime, timedelta
    
    stats = {
        "total_people": count_people(db),
        "total_reference_images": db.query(ReferenceImage).count(),
        "pending_annotations": db.query(AnnotationQueue).filter(
            AnnotationQueue.status == "pending"
        ).count(),
        "total_detections_24h": db.query(DetectionLog).filter(
            DetectionLog.created_at >= datetime.utcnow() - timedelta(hours=24)
        ).count(),
...
async def people_list(
    ...
    for person in people:
        person.reference_image_count = db.query(ReferenceImage).filter(
            ReferenceImage.person_id == person.id
        ).count()
